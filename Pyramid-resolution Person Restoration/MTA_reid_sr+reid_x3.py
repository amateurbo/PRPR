...
models_path = '/home/gpu/code/SwinIR-main/model_zoo/MTA_reid/x3/' #model path
...
sr_task = 'python main_nogt_swinir_MTA_reid.py --task classical_sr --scale 3 --training_patch_size 64 --window_size 8 --model_path '
set = 'x3'
dataname = 'MTA_reid_'
...
for i in  range(len(dir_list)):
    model_path = os.path.join(models_path, dir_list[i])
    sr_query = sr_task + model_path + query
    sr_train = sr_task + model_path + train
    sr_test = sr_task + model_path + test
    reid_log = ' data.save_dir /mnt/data/code/reidlog/MTA_reid/x3/osnet_x1_0_test_x3_MTA_reid_softmax_cosinelr'

    # # 超分
    # os.system(sr_query)
    # os.system(sr_train)
    #将其他数据复制过来
    path = 'results/swinir_classical_sr_x3_testsets/MTA_reid/'
