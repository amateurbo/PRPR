...
def main():
    ...
    parser.add_argument('--training_patch_size', type=int, default=64, help='patch size used in training SwinIR. '
                                       ...
                                       'Images are NOT tested patch by patch.')
    parser.add_argument('--large_model', action='store_true', help='use large model, only provided for real image sr')
    parser.add_argument('--model_path', type=str, default='/mnt/data/code/superresolution/market1501_s64w8_x34_x2_x2/models/900_E.pth')
    parser.add_argument('--folder_lq', type=str, default='/mnt/data/mlr_datasets/MLR_Market1501/market1501/query', help='input low-quality test image folder')
    parser.add_argument('--folder_gt', type=str, default='/mnt/data/mlr_datasets/MLR_Market1501/market1501/query', help='input ground-truth test image folder')
    args = parser.parse_args()
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        # read image
        print(path)
        imgname, img_lq, flag= get_image_pair(args, path)  # image to HWC-BGR, float32

        # if(flag == 1):
        #     shutil.copy(path, f'{save_dir}/{imgname}.png')
        #     continue
        if(flag == 2):
            h, w = img_lq.shape[:-1]
            img_lq = cv2.resize(img_lq, (h*2, w*2), interpolation=cv2.INTER_CUBIC)
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        save_dir = f'../results_MTA_reid_new/swinir_{args.task}_x{args.scale}_{args.folder_lq}'
