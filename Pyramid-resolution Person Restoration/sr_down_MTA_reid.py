        ...
        for subdir in subdirs:
            ...
            if scale == 1:
                for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
                    ...
                    cv2.imwrite(f'{savedir_hr}/{imgname}.png', img_hr)
                    cv2.imwrite(f'{savedir_x2}/{imgname}.png', img_x2)
                    cv2.imwrite(f'{savedir_x3}/{imgname}.png', img_x3)
                    cv2.imwrite(f'{savedir_x4}/{imgname}.png', img_x4)
                    cv2.imwrite(f'{savedir_x8}/{imgname}.png', img_x8)
...
def reid(datapath):
    ...
    for dirlist in dirlists:
        ...
        root = ' data.root ' + path
        reid_task = 'python /mnt/data/code/deep-person-reid-master/scripts/MTA_reid_new.py ' \
                    '--config-file /mnt/data/code/deep-person-reid-master/configs/MTA_reid.yaml '
        reid_log = ' data.save_dir /mnt/data/code/reidlog/MTA_reid_new/' + dirlist
