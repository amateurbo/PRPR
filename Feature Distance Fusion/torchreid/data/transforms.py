...
def build_transforms(
    ...
    **kwargs
    ...
    transform_tr += [Resize((height, width), interpolation=PIL.Image.BICUBIC)]
    ...
    if 'body_blur' in transforms:
        print('+ body blur')
        transform_tr += [BodyBlur(probability=0.5, bodyparts=['body'], blurparam=5, blendparam=1, randpart=False, partnum=1)]
    ...
    transform_te = Compose([
        Resize((height, width)),
        # Resize((height, width), interpolation=PIL.Image.BICUBIC),
        ToTensor(),
        normalize,
    ])
